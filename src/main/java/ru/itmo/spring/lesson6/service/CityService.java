...
public class CityService {
    ...
    public List<CityResponseDto> findAllCities() {
        return cityRepository.findAllWithRegion().stream()
                .map(CityResponseDto::fromEntity)
                .collect(Collectors.toList());
    }

    public CityResponseDto findCityById(Long id) {
        City city = cityRepository.findByIdWithRegion(id)
                .orElseThrow(() -> new RuntimeException("Город с ID " + id + " не найден"));
        return CityResponseDto.fromEntity(city);
    }

    public CityResponseDto findCityByCode(Integer code) {
        City city = cityRepository.findByCode(code)
                .orElseThrow(() -> new RuntimeException("Город с кодом " + code + " не найден"));
        return CityResponseDto.fromEntity(city);
    }

    public List<CityResponseDto> searchCitiesByName(String name) {
        return cityRepository.findByNameRuContainingIgnoreCase(name).stream()
                .map(CityResponseDto::fromEntity)
                .collect(Collectors.toList());
    }

    public List<CityResponseDto> findCitiesByRegionCode(Integer regionCode) {
        Region region = regionRepository.findByCode(regionCode)
                .orElseThrow(() -> new RuntimeException("Регион с кодом " + regionCode + " не найден"));

        return cityRepository.findAllByRegion(region).stream()
                .map(CityResponseDto::fromEntity)
                .collect(Collectors.toList());
    }
    ...
    public CityResponseDto createCity(CityRequestDto request) {
        ...
        Region region = regionRepository.findByCode(request.getRegionCode())
                .orElseThrow(() -> new RuntimeException("Регион с кодом " + request.getRegionCode() + " не найден"));
...
//    }
//
//    public void viewAllCitiesConsole() {
//        System.out.println("\n--- Список всех городов ---");
...
//    }
//
//    public void viewAllRegionsConsole() {
...
//
//    @Transactional
//    public void deleteRegionConsole() {
//        System.out.println("\nУДАЛЕНИЕ РЕГИОНА");
//
